...
function loadFolioData(data,page){
	...
	$("#about_me").append('<center> <div id="name"><h1>'+name+'</h1></div><img src="../static/uploads/'+username+'/new.png"></center<br><br><br><br>> <form method="POST" enctype="multipart/form-data" action="/<username>/"><input type="file" name="file[]" multiple=""><input type="hidden" name="uzernaem" value="'+username+'"><input type="submit" value="add"></form><form id="blurb_form"><button type="button" id="blurb_save" name="Save">Save</button> <textarea type="text" id="blurb" resize="false" placeholder="Write about yourself here."></textarea></form>');
